...
New-DscDashboardCustomHeader -Text $Hostname -icon 'chart_bar'
    ...
    New-UDCollapsible -Items {
        ...
        New-UDRow {
            ...
            New-UDColumn -SmallSize 12 -MediumSize 12 -LargeSize 6 {
                        New-UDTable -Title "Configuration Information" -Headers @(" ", " ") -Endpoint {
                            $configData =@{}
                            $data.StatusData.MetaData.Split(';') | % {$row = $_; $configData.add(($row -split ':',2)[0].trim(),($row -split ':',2)[1].trim())}
                            $configData | Out-UDTableData -Property @("Name", "Value")
                        }
